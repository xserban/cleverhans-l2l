...
from cleverhansl2l.compat import reduce_mean, reduce_prod
from cleverhansl2l.model import Model
from cleverhansl2l.serial import PicklableVariable as PV
from cleverhansl2l.utils import ordered_union


class PicklableGetVariable(object):
  def __init__(self, initializer, name, scope, shape=None, *args, **kwargs):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
      if name is 'b':
        self.var = tf.get_variable(name=name,
  ...
  def __getstate__(self):
    ...
    if sess is None:
      raise RuntimeError("PicklableGetVariable requires a default "
                         "TensorFlow session")
...
class PicklableModelGet(Model):
  ...
  def __init__(self):
    super(PicklableModelGet, self).__init__()
...
class MLPGet(PicklableModelGet):
  """
...
class Linear(Layer):
  ...
  def set_input_shape(self, input_shape):
    ...
    elif self.init_mode == "custom_variable":
      init=None
    else:
      raise ValueError(self.init_mode)
    self.W=PicklableGetVariable(init, "W", self.scope, [dim, self.num_hid])
    if self.use_bias:
      self.b=PicklableGetVariable((np.zeros((self.num_hid,))
